...
export async function listClubsForUser(userId) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `SELECT c.id, c.code, c.name, c.image, c.cover_image, c.owner_id, m.chips, m.role
       FROM club_members m JOIN clubs c ON c.id = m.club_id
       WHERE m.user_id = $1 ORDER BY m.joined_at ASC`,
      [userId]
    );
    return rows.map((r) => ({ id: r.id, code: r.code, name: r.name, image: r.image || null, coverImage: r.cover_image || null, chips: r.chips, isOwner: r.owner_id === userId }));
  }
  return mem.members
    .filter((m) => m.user_id === userId)
    .map((m) => {
      const c = mem.clubs.find((c) => c.id === m.club_id);
      return { id: c.id, code: c.code, name: c.name, image: c.image || null, coverImage: c.cover_image || null, chips: m.chips, isOwner: c.owner_id === userId };
    });
}
...
export async function recordRakeEvent(clubId, memberUserId, tableCode, grossAmount) {
  ...
  // O rake INTEIRO (bruto) entra como RK — uma ficha separada da ficha
  // normal do clube, exatamente pra não confundir no acerto. Rakeback e
  // comissão saem do RK e viram ficha DE VERDADE na hora (o jogador/
  // agente precisa jogar com isso), pagos imediatamente, não só
  // anotados pra pagar depois. O que sobra (clubResult) fica no RK,
  // separado do saldo normal do clube.
  await adjustClubRkBalance(clubId, grossAmount);
  if (rakebackAmount > 0) {
    await adjustMemberChips(clubId, memberUserId, rakebackAmount);
    await adjustClubRkBalance(clubId, -rakebackAmount);
  }
  if (commissionAmount > 0 && agentWalletId) {
    ...
    if (agentRow) {
      ...
      await adjustClubRkBalance(clubId, -commissionAmount);
    }
  }
...
export async function getAgentRakeTable(clubId, from, to) {
  ...
  for (const a of agents) {
    ...
    out.push({
      id: a.id, username: a.username, status: a.status, commissionRate: rate, memberCount,
