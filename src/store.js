...
const mem = {
  ...
  notifications: [], // {id, user_id, kind, title, body, from_label, created_at}
  nextUserId: 1,
  nextClubId: 1,
  nextClubTableId: 1,
  nextLedgerId: 1,
  ...
  if (hasDatabase) {
    const { rows } = await pool.query(
      `INSERT INTO quick_wallets (user_id, chips) VALUES ($1, $2)
       ON CONFLICT (user_id) DO NOTHING RETURNING chips, diamonds, last_daily_claim`,
      [userId, QUICK_WALLET_START]
    );
    if (rows[0]) return { chips: rows[0].chips, diamonds: rows[0].diamonds, lastDailyClaim: rows[0].last_daily_claim };
    const existing = await pool.query("SELECT chips, diamonds, last_daily_claim FROM quick_wallets WHERE user_id = $1", [userId]);
    const r = existing.rows[0];
    return r ? { chips: r.chips, diamonds: r.diamonds, lastDailyClaim: r.last_daily_claim } : { chips: QUICK_WALLET_START, diamonds: 0, lastDailyClaim: null };
  }
  let w = mem.quickWallets.find((w) => w.user_id === userId);
  if (!w) { w = { user_id: userId, chips: QUICK_WALLET_START, diamonds: 0, last_daily_claim: null }; mem.quickWallets.push(w); }
  return { chips: w.chips, diamonds: w.diamonds || 0, lastDailyClaim: w.last_daily_claim };
}
...
// Compra de diamantes com Royalle Coin — economia fictícia, sem dinheiro
// real (mesmo espírito do resto do app). Retorna null se saldo insuficiente.
export async function buyDiamonds(userId, coinCost, diamondAmount) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `UPDATE quick_wallets SET chips = chips - $2, diamonds = diamonds + $3
       WHERE user_id = $1 AND chips >= $2 RETURNING chips, diamonds`,
      [userId, coinCost, diamondAmount]
    );
    return rows[0] || null;
  }
  const w = mem.quickWallets.find((w) => w.user_id === userId);
  if (!w || w.chips < coinCost) return null;
  w.chips -= coinCost;
  w.diamonds = (w.diamonds || 0) + diamondAmount;
  return { chips: w.chips, diamonds: w.diamonds };
}
...
export async function createUser(username, passwordHash, avatar) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      "INSERT INTO users (username, password_hash, avatar) VALUES ($1,$2,$3) RETURNING id, username, avatar, nickname",
      [username, passwordHash, avatar]
    );
    return rows[0];
  }
  if (mem.users.find((u) => u.username.toLowerCase() === username.toLowerCase())) return null;
  const user = { id: mem.nextUserId++, username, password_hash: passwordHash, avatar, nickname: null };
  mem.users.push(user);
  return { id: user.id, username: user.username, avatar: user.avatar, nickname: user.nickname };
}

// Apelido separado do login — pode ser trocado, o username (login) não.
export async function setNickname(userId, nickname) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      "UPDATE users SET nickname = $2 WHERE id = $1 RETURNING id, username, avatar, nickname",
      [userId, nickname]
    );
    return rows[0] || null;
  }
  ...
  if (!u) return null;
  u.nickname = nickname;
  return { id: u.id, username: u.username, avatar: u.avatar, nickname: u.nickname };
}
...
export async function findUserById(id) {
  if (hasDatabase) {
    const { rows } = await pool.query("SELECT id, username, avatar, nickname FROM users WHERE id = $1", [id]);
    return rows[0] || null;
  }
  const u = mem.users.find((u) => u.id === id);
  return u ? { id: u.id, username: u.username, avatar: u.avatar, nickname: u.nickname } : null;
}

export async function createClub({ code, name, ownerId, smallBlind, bigBlind, buyIn, rakePercent, coverId = "rio" }) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `INSERT INTO clubs (code, name, owner_id, small_blind, big_blind, buy_in, rake_percent, treasury_chips, cover_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7,1000,$8) RETURNING *`,
      [code, name, ownerId, smallBlind, bigBlind, buyIn, rakePercent, coverId]
    );
    return rows[0];
  }
  const club = { id: mem.nextClubId++, code, name, owner_id: ownerId, small_blind: smallBlind, big_blind: bigBlind, buy_in: buyIn, rake_percent: rakePercent, treasury_chips: 1000, cover_id: coverId };
  mem.clubs.push(club);
  return club;
}

// Troca a capa do clube (escolhida pelo dono entre as opções pré-definidas).
export async function setClubCover(clubId, coverId) {
  if (hasDatabase) {
    const { rows } = await pool.query("UPDATE clubs SET cover_id = $2 WHERE id = $1 RETURNING *", [clubId, coverId]);
    return rows[0] || null;
  }
  const club = mem.clubs.find((c) => c.id === clubId);
  if (!club) return null;
  club.cover_id = coverId;
  return club;
}
...
export async function listMembers(clubId) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `SELECT u.id, u.username, u.avatar, u.nickname, m.chips, m.role
  ...
  return mem.members
    ...
    .map((m) => {
      ...
      return { id: u.id, username: u.username, avatar: u.avatar, nickname: u.nickname, chips: m.chips, role: m.role };
    });
}
...
// ---- notificações (tela "Mensagens") ----

export async function createNotification({ userId, kind = "notice", title, body, fromLabel = null }) {
  if (hasDatabase) {
    await pool.query(
      `INSERT INTO notifications (user_id, kind, title, body, from_label) VALUES ($1,$2,$3,$4,$5)`,
      [userId, kind, title, body, fromLabel]
    );
    return;
  }
  mem.notifications.push({ id: mem.nextNotificationId++, user_id: userId, kind, title, body, from_label: fromLabel, created_at: new Date().toISOString() });
}
...
export async function listNotifications(userId) {
  ...
  return mem.notifications.filter((n) => n.user_id === userId).slice().reverse();
}
...
export async function deleteNotification(userId, id) {
  ...
  const idx = mem.notifications.findIndex((n) => n.id === id && n.user_id === userId);
  if (idx >= 0) mem.notifications.splice(idx, 1);
}
