...
const mem = {
  ...
  nextRakeEventId: 1,
  nextRakeClosingId: 1,
  clubMessages: [], // {id, club_id, type, content, image, expires_at, created_by, created_at}
  nextClubMessageId: 1,
};
...
export async function createJoinRequest(clubId, userId, agentUsername) {
  if (hasDatabase) {
    await pool.query(
      `INSERT INTO club_join_requests (club_id, user_id, agent_username) VALUES ($1,$2,$3)
       ON CONFLICT (club_id, user_id) DO UPDATE SET agent_username = EXCLUDED.agent_username`,
      [clubId, userId, agentUsername || null]
    );
    return;
  }
  const existing = mem.joinRequests.find((r) => r.club_id === clubId && r.user_id === userId);
  if (existing) existing.agent_username = agentUsername || null;
  else mem.joinRequests.push({ id: mem.nextJoinRequestId++, club_id: clubId, user_id: userId, agent_username: agentUsername || null });
}

// Código do agente (username dele) que a pessoa informou ao pedir
// entrada — usado na hora de aprovar pra já vincular automaticamente.
export async function getJoinRequestAgentUsername(clubId, userId) {
  if (hasDatabase) {
    const { rows } = await pool.query("SELECT agent_username FROM club_join_requests WHERE club_id=$1 AND user_id=$2", [clubId, userId]);
    return rows[0]?.agent_username || null;
  }
  return mem.joinRequests.find((r) => r.club_id === clubId && r.user_id === userId)?.agent_username || null;
}
...
// Apelido — nome exibido pro resto do app, separado do login. Não tem
// as mesmas restrições de unicidade do username (dois jogadores podem
// ter o mesmo apelido).
...
export async function createClub({ code, name, ownerId, smallBlind, bigBlind, buyIn, rakePercent }) {
  ...
  const club = { id: mem.nextClubId++, code, name, owner_id: ownerId, small_blind: smallBlind, big_blind: bigBlind, buy_in: buyIn, rake_percent: rakePercent, treasury_chips: 10000, level: 0, level_expires_at: null, rk_balance: 0, jackpot_enabled: false, jackpot_announce: false, jackpot_percent: 1, jackpot_balance: 0 };
  mem.clubs.push(club);
  return club;
}
...
// Saldo do Jackpot — alimentado pela % do rake (automático) e também
// por injeção manual de fichas da tesouraria (o dono "coloca" um valor
// direto, ex: 2.000 fichas pra dar um empurrão inicial no prêmio).
export async function adjustClubJackpotBalance(clubId, delta) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      "UPDATE clubs SET jackpot_balance = GREATEST(jackpot_balance + $2, 0) WHERE id=$1 RETURNING jackpot_balance",
      [clubId, delta]
    );
    return rows[0]?.jackpot_balance ?? 0;
  }
  const c = mem.clubs.find((c) => c.id === clubId);
  if (!c) return 0;
  if (c.jackpot_balance === undefined) c.jackpot_balance = 0;
  c.jackpot_balance = Math.max(0, c.jackpot_balance + delta);
  return c.jackpot_balance;
}
...
export async function recordRakeEvent(clubId, memberUserId, tableCode, grossAmount) {
  ...
  if (rakebackAmount > 0) {
    ...
    // Carreira: sem isso, o jogador nunca via o rakeback que ganhou —
    // ele recebia a ficha de verdade, mas o extrato ficava mudo sobre
    // de onde ela veio.
    await recordHandLedger(clubId, null, memberUserId, rakebackAmount);
  }
  if (commissionAmount > 0 && agentWalletId) {
    ...
    if (agentRow) {
      ...
      await recordHandLedger(clubId, null, agentRow.user_id, commissionAmount);
    }
  }

  // Jackpot — se estiver ligado, uma % do que sobrou pro clube (depois
  // de já pagar rakeback e comissão) sai da tesouraria e vai pro
  // saldo do jackpot, em vez de ficar como lucro normal do clube.
  let jackpotCut = 0;
  const clubForJackpot = await getClubById(clubId);
  if (clubForJackpot?.jackpot_enabled && Number(clubForJackpot.jackpot_percent) > 0 && clubResult > 0) {
    jackpotCut = Math.floor((clubResult * Number(clubForJackpot.jackpot_percent)) / 100);
    if (jackpotCut > 0) {
      await adjustClubTreasury(clubId, -jackpotCut);
      await adjustClubJackpotBalance(clubId, jackpotCut);
    }
  }
...
// ============================================================
// MENSAGENS/AVISOS DO CLUBE — dono/gestor manda pros membros, com um
// prazo de término obrigatório. "Exibir sempre ao entrar no Clube até
// o término definido" = fica valendo até expires_at, sem precisar
// reenviar.
// ============================================================
export async function createClubMessage({ clubId, type, content, image, expiresAt, createdBy }) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `INSERT INTO club_messages (club_id, type, content, image, expires_at, created_by) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [clubId, type, content || null, image || null, expiresAt, createdBy]
    );
    return rows[0];
  }
  const row = { id: mem.nextClubMessageId++, club_id: clubId, type, content: content || null, image: image || null, expires_at: expiresAt, created_by: createdBy, created_at: new Date().toISOString() };
  mem.clubMessages.push(row);
  return row;
}

export async function listClubMessages(clubId, activeOnly = false) {
  const now = Date.now();
  if (hasDatabase) {
    const { rows } = await pool.query(
      `SELECT * FROM club_messages WHERE club_id=$1 ${activeOnly ? "AND expires_at > now()" : ""} ORDER BY created_at DESC`,
      [clubId]
    );
    return rows;
  }
  return mem.clubMessages
    .filter((m) => m.club_id === clubId && (!activeOnly || new Date(m.expires_at).getTime() > now))
    .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
}

export async function deleteClubMessage(clubId, messageId) {
  if (hasDatabase) {
    await pool.query("DELETE FROM club_messages WHERE club_id=$1 AND id=$2", [clubId, messageId]);
    return;
  }
  const i = mem.clubMessages.findIndex((m) => m.club_id === clubId && m.id === Number(messageId));
  if (i >= 0) mem.clubMessages.splice(i, 1);
}

// ============================================================
// JACKPOT — configuração por clube (ligar/desligar, anúncio de
// premiação, % de cada pote que alimenta o jackpot). O gatilho/entrega
// do prêmio em si fica pra uma etapa futura; por enquanto é só a
// configuração, como foi pedido.
// ============================================================
export async function setClubJackpotConfig(clubId, { enabled, announce, percent }) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `UPDATE clubs SET jackpot_enabled=$1, jackpot_announce=$2, jackpot_percent=$3 WHERE id=$4
       RETURNING jackpot_enabled, jackpot_announce, jackpot_percent, jackpot_balance`,
      [enabled, announce, percent, clubId]
    );
    return rows[0];
  }
  const c = mem.clubs.find((c) => c.id === clubId);
  if (!c) return null;
  c.jackpot_enabled = enabled; c.jackpot_announce = announce; c.jackpot_percent = percent;
  if (c.jackpot_balance === undefined) c.jackpot_balance = 0;
  return { jackpot_enabled: c.jackpot_enabled, jackpot_announce: c.jackpot_announce, jackpot_percent: c.jackpot_percent, jackpot_balance: c.jackpot_balance };
}
