...
export async function addTournamentEntry(tournamentId, userId, chips) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `INSERT INTO tournament_entries (tournament_id, user_id, chips) VALUES ($1,$2,$3)
       ON CONFLICT (tournament_id, user_id) DO NOTHING RETURNING *`,
      [tournamentId, userId, chips]
    );
    return rows[0] || null;
  }
  ...
  const e = {
    ...
    chips, rebuys: 0, rank: null, prize: 0, eliminated_at: null, registered_at: new Date().toISOString(),
  };
...
export async function listClubsForUser(userId) {
  if (hasDatabase) {
    const { rows } = await pool.query(
      `SELECT c.id, c.code, c.name, c.image, c.owner_id, m.chips, m.role
       FROM club_members m JOIN clubs c ON c.id = m.club_id
       WHERE m.user_id = $1 ORDER BY m.joined_at ASC`,
      [userId]
    );
    return rows.map((r) => ({ id: r.id, code: r.code, name: r.name, image: r.image || null, chips: r.chips, isOwner: r.owner_id === userId }));
  }
  return mem.members
    .filter((m) => m.user_id === userId)
    .map((m) => {
      const c = mem.clubs.find((c) => c.id === m.club_id);
      return { id: c.id, code: c.code, name: c.name, image: c.image || null, chips: m.chips, isOwner: c.owner_id === userId };
    });
}
