...
export async function migrate() {
  ...
  `);
  // Migração pra bancos que já existiam antes da coluna nickname.
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS clubs (
      ...
      cover_id TEXT NOT NULL DEFAULT 'rio',
      created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    );
  `);
  // Migração pra bancos que já existiam antes da coluna cover_id.
  await pool.query(`ALTER TABLE clubs ADD COLUMN IF NOT EXISTS cover_id TEXT NOT NULL DEFAULT 'rio';`);
  ...
  await pool.query(`ALTER TABLE quick_wallets ADD COLUMN IF NOT EXISTS diamonds BIGINT NOT NULL DEFAULT 0;`);
  ...
  `);
  // Notificações do jogador (fichas recebidas, avisos de boas-vindas,
  // notificações de clube) — tela "Mensagens".
  await pool.query(`
