...
import { potLimitMaxRaiseTotal } from "./pokerEngine.js";
...
export function pickBotAction(table, botId) {
  ...
  // Tamanho do raise: usa o tamanho do PRÓPRIO POTE como referência de
  // "raise de peso" (a mesma fórmula que a engine usa pro teto de
  // Pot-Limit, só que aqui sem o limite do stack — é só uma régua de
  // quanto seria um raise pot-sized nesse momento). Antes, o bot sempre
  // acrescentava só "minRaise ou 2x BB" por cima da aposta atual — isso
  // fazia os raises ficarem pequenos e meio aleatórios conforme a mão
  // avançava (ex: 1000, depois só +100 pra 1100, depois +200 pra 1300),
  // em vez de crescerem de um jeito consistente com o pote. Mirando
  // entre 55% e 90% do caminho até um raise pot-sized (e nunca passando
  // do teto real da mesa — o stack no No-Limit, o próprio pote no
  // Pot-Limit), o raise sempre cresce de forma coerente com o jogo.
  const potSizedTotal = potLimitMaxRaiseTotal(table.pot, table.currentBet, p.roundBet, Infinity);
  const spread = Math.max(0, potSizedTotal - legalMinRaiseTotal);
  const sizedRaiseTotal = () => {
    const target = legalMinRaiseTotal + spread * (0.55 + Math.random() * 0.35);
    return Math.round(Math.max(legalMinRaiseTotal, Math.min(target, cap)));
  };

  if (toCall <= 0) {
    // Pode passar de graça. Às vezes aposta se a mão parece boa.
    if (strength > 0.72 && noise < 0.55 && p.chips > table.bigBlind) {
      return { action: "raise", amount: sizedRaiseTotal() };
    }
    return { action: "check" };
  }
