...
// Omaha (PLO): diferente do Hold'em, é OBRIGATÓRIO usar exatamente 2
// cartas da mão do jogador + exatamente 3 do board — nunca 1, 3 ou mais.
// Testamos todas as combinações possíveis dentro dessa regra.
function bestOfOmaha(holeCards, community) {
  let best = null;
  for (const hole2 of combinations(holeCards, 2)) {
    for (const board3 of combinations(community, 3)) {
      const s = evaluate5([...hole2, ...board3]);
      if (!best || compareScores(s, best) > 0) best = s;
    }
  }
  return best;
}
// Quantas cartas na mão cada variante recebe, e como ela é avaliada no
// showdown — ponto único que o resto do motor consulta.
const VARIANT_HOLE_CARDS = { holdem: 2, plo4: 4, plo5: 5, plo6: 6 };
function evaluateHand(variant, holeCards, community) {
  return variant === "holdem" ? bestOf7([...holeCards, ...community]) : bestOfOmaha(holeCards, community);
}
...
export class PokerTable {
  constructor({ smallBlind = 25, bigBlind = 50, rakePercent = 0, variant = "holdem" } = {}) {
    ...
    this.variant = variant; // "holdem" | "plo4" | "plo5" | "plo6"
  ...
  startHand() {
    ...
    this.players.forEach((p) => { if (p.inHand) { const n = VARIANT_HOLE_CARDS[this.variant] || 2; p.cards = Array.from({ length: n }, () => this.deck.pop()); } });
  ...
  resolveShowdownOrFold() {
    ...
    if (contenders.length === 1) {
      ...
      const scored = {};
      contenders.forEach((p) => { scored[p.id] = evaluateHand(this.variant, p.cards, this.community); });
