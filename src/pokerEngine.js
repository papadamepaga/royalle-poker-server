...
export class PokerTable {
  constructor({ smallBlind = 25, bigBlind = 50, rakePercent = 0, variant = "holdem", maxPlayers = 9 } = {}) {
    ...
    this.maxPlayers = maxPlayers; // quantos assentos a mesa tem — usado pra desenhar as vagas vazias
  ...
  addPlayer(id, name, chips, isBot = false, seat = null) {
    ...
    // "seat" é só a posição VISUAL escolhida (pro botão "+" de sentar
    // num lugar específico) — nunca mexe na ordem de turno/dealer, que
    // continua sendo a ordem de entrada na mesa (this.players).
    const taken = new Set(this.players.map((p) => p.seat));
    let assigned = seat;
    if (assigned == null || assigned < 0 || assigned >= this.maxPlayers || taken.has(assigned)) {
      assigned = 0;
      while (taken.has(assigned) && assigned < this.maxPlayers) assigned++;
    }
    this.players.push({ id, name, chips, cards: [], folded: false, allIn: false, inHand: false, roundBet: 0, totalBet: 0, connected: true, isBot, seat: assigned });
  }
  ...
  resolveShowdownOrFold() {
    ...
    if (contenders.length === 1) {
      ...
      pots.forEach((potObj, idx) => {
        ...
        // Se só UM jogador colocou dinheiro nessa camada do pote, isso não
        // é uma disputa de verdade — é sobra de aposta que ninguém mais
        // podia pagar (ex: eu vou all-in de 2000 mas o adversário só tinha
        // 500; os 1500 que sobram voltam pra mim sem ninguém ter "jogado"
        // contra eles). Rake NUNCA pode ser cobrado dessa devolução —
        // só do que foi de fato disputado entre 2+ jogadores.
        const isUncalledReturn = Object.keys(potObj.contributions).length === 1;
        const rakeAmount = (!isUncalledReturn && this.rakePercent > 0) ? Math.floor((potObj.amount * this.rakePercent) / 100) : 0;
