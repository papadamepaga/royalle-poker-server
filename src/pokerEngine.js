  ...
  resolveShowdownOrFold() {
    ...
    if (contenders.length === 1) {
      ...
      pots.forEach((potObj, idx) => {
        ...
        // Do rake total, uma fatia fixa de pontos percentuais do pote é do
        // app (nunca mais do que o rake total configurado) — o resto é do
        // clube.
        const platformPercent = Math.min(PLATFORM_RAKE_POINTS, this.rakePercent);
        const platformAmount = this.rakePercent > 0 ? Math.floor((potObj.amount * platformPercent) / 100) : 0;
        const clubAmount = rakeAmount - platformAmount;
        this.pendingRake += clubAmount;
        this.pendingPlatformRake += platformAmount;
