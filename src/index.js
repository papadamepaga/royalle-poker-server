...
async function maybeRecordHandLedger(rt, code) {
  ...
  // Mesas de torneio têm código tipo "CLUBE#T1-0" — o "1-0" não é um ID
  // de mesa de verdade, então "Number(...)" viraria NaN e quebraria o
  // registro. Carreira de torneio ainda não é rastreada por mesa, então
  // só grava normalmente pras mesas de clube de verdade.
  const tableId = (!rt.isTournament && code.includes("#")) ? Number(code.split("#")[1]) : null;
...
async function handleMessage(ws, msg, ctx) {
  ...
  if (type === "rebuy_tournament") {
    ...
    const code = (msg.code || "").toUpperCase();
    const rt = runtime.get(code);
    if (!rt?.isTournament || !rt.table) return ctx.reply({ ok: false, error: "Mesa de torneio não encontrada." });
    const t = await getTournamentById(rt.tournamentId);
    if (!t) return ctx.reply({ ok: false, error: "Torneio não encontrado." });
    if (!t.rebuy_allowed) return ctx.reply({ ok: false, error: "Esse torneio não permite recompra." });
    if (t.status !== "running") return ctx.reply({ ok: false, error: "Esse torneio já terminou." });
    ...
    ctx.reply({ ok: true, chips: Number(t.starting_chips) });
    broadcastTable(code);
    return;
  }
