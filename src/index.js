...
import {
  AVATAR_OPTIONS, createUser, findUserByUsername, findUserById, setNickname,
  ...
  createClubMessage, listClubMessages, deleteClubMessage, setClubJackpotConfig, adjustClubJackpotBalance,
  ...
  createJoinRequest, hasJoinRequest, listJoinRequests, removeJoinRequest, getJoinRequestAgentUsername,
...
function publicClub(club, viewerIsOwner, extra = {}) {
  ...
  const base = {
    ...
    jackpotAnnounce: !!club.jackpot_announce,
    jackpotPercent: Number(club.jackpot_percent || 0),
...
async function handleMessage(ws, msg, ctx) {
  ...
  if (type === "signup") {
    ...
    // needsNickname: true — o login acabou de ser criado, ainda não
    // definiu o apelido (nome que aparece pro resto do app). O cliente
    // deve pedir o apelido antes de entrar de verdade.
    ctx.reply({ ok: true, token, user: { id: user.id, username: user.username, avatar: user.avatar, avatarImage: null, nickname: null }, needsNickname: true });
    return;
  }
  ...
  // Define o apelido (separado do login) — chamado logo depois do
  // cadastro, antes de entrar de fato no app.
  if (type === "set_nickname") {
    if (!requireAuth(ws, ctx)) return;
    const nickname = (msg.nickname || "").trim();
    if (nickname.length < 2 || nickname.length > 24) return ctx.reply({ ok: false, error: "O apelido precisa ter entre 2 e 24 caracteres." });
    await setNickname(ws.userId, nickname);
    ctx.reply({ ok: true, nickname });
    return;
  }
  ...
  if (type === "join_club") {
    ...
    // antes de dar acesso de verdade ao clube. Código do agente é
    // opcional — só guarda pra vincular automaticamente quando aprovar.
    await createJoinRequest(club.id, ws.userId, (msg.agentCode || "").trim() || null);
  ...
  if (type === "approve_join_request") {
    ...
    // Se a pessoa informou um código de agente ao pedir entrada, vincula
    // agora — só se esse agente realmente existir e estiver ativo NESSE
    // clube (nunca falha a aprovação por causa disso).
    const agentUsername = await getJoinRequestAgentUsername(club.id, target.id);
    if (agentUsername) {
      const agentUser = await findUserByUsername(agentUsername);
      if (agentUser) {
        const agentWallet = await getAgentWallet(club.id, agentUser.id);
        if (agentWallet && agentWallet.status === "active") {
          await linkMemberToAgent(club.id, target.id, agentWallet.id, ws.userId);
        }
      }
    }
  ...
  // ============================================================
  // MENSAGENS/AVISOS DO CLUBE
  // ============================================================
  if (type === "create_club_message") {
    if (!requireAuth(ws, ctx)) return;
    const club = await getClubByCode((msg.code || "").toUpperCase());
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    const me = await getMember(club.id, ws.userId);
    if (!me || (me.role !== "owner" && me.role !== "agent")) return ctx.reply({ ok: false, error: "Sem permissão." });
    const msgType = ["texto", "imagem", "aviso"].includes(msg.msgType) ? msg.msgType : "texto";
    if (!msg.expiresAt) return ctx.reply({ ok: false, error: "Preencha o prazo de término." });
    const expiresAt = new Date(msg.expiresAt);
    if (isNaN(expiresAt.getTime()) || expiresAt.getTime() <= Date.now()) {
      return ctx.reply({ ok: false, error: "O prazo de término precisa ser uma data futura." });
    }
    if (msgType !== "imagem" && !(msg.content || "").trim()) return ctx.reply({ ok: false, error: "Escreva o texto da mensagem." });
    if (msgType === "imagem" && !msg.image) return ctx.reply({ ok: false, error: "Escolha uma imagem." });
    const created = await createClubMessage({
      clubId: club.id, type: msgType, content: (msg.content || "").trim() || null,
      image: msg.image || null, expiresAt: expiresAt.toISOString(), createdBy: ws.userId,
    });
    ctx.reply({ ok: true, message: created });
    return;
  }

  if (type === "list_club_messages") {
    if (!requireAuth(ws, ctx)) return;
    const club = await getClubByCode((msg.code || "").toUpperCase());
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    const me = await getMember(club.id, ws.userId);
    if (!me) return ctx.reply({ ok: false, error: "Sem permissão." });
    const isAdmin = me.role === "owner" || me.role === "agent";
    // Dono/gestor vê tudo (inclusive expirado, pra gerenciar); membro
    // comum só vê o que ainda está valendo.
    const messages = await listClubMessages(club.id, !isAdmin);
    ctx.reply({ ok: true, messages });
    return;
  }

  if (type === "delete_club_message") {
    if (!requireAuth(ws, ctx)) return;
    const club = await getClubByCode((msg.code || "").toUpperCase());
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    const me = await getMember(club.id, ws.userId);
    if (!me || (me.role !== "owner" && me.role !== "agent")) return ctx.reply({ ok: false, error: "Sem permissão." });
    await deleteClubMessage(club.id, msg.messageId);
    ctx.reply({ ok: true });
    return;
  }

  // ============================================================
  // JACKPOT — só configuração por enquanto (ligar/desligar, anúncio,
  // % de cada pote que alimenta o jackpot).
  // ============================================================
  if (type === "set_jackpot_config") {
    ...
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    const me = await getMember(club.id, ws.userId);
    if (!me || me.role !== "owner") return ctx.reply({ ok: false, error: "Só o dono do clube configura o Jackpot." });
    const percent = Number(msg.percent);
    if (!isFinite(percent) || percent < 0 || percent > 10) return ctx.reply({ ok: false, error: "% inválida (0 a 10)." });
    const updated = await setClubJackpotConfig(club.id, { enabled: !!msg.enabled, announce: !!msg.announce, percent });
    ctx.reply({ ok: true, config: updated });
    return;
  }

  // Injeta fichas da tesouraria do clube direto no Jackpot — ex: "criar
  // o jack com 2.000 de empurrão inicial". Some da tesouraria, some pro
  // saldo do jackpot (não cria dinheiro do nada).
  if (type === "add_jackpot_funds") {
    if (!requireAuth(ws, ctx)) return;
    const club = await getClubByCode((msg.code || "").toUpperCase());
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    const me = await getMember(club.id, ws.userId);
    if (!me || me.role !== "owner") return ctx.reply({ ok: false, error: "Só o dono do clube alimenta o Jackpot." });
    const amount = Math.floor(Number(msg.amount));
    if (!isFinite(amount) || amount <= 0) return ctx.reply({ ok: false, error: "Valor inválido." });
    if (Number(club.treasury_chips) < amount) return ctx.reply({ ok: false, error: "Saldo do clube insuficiente." });
    const treasuryChips = await adjustClubTreasury(club.id, -amount);
    const jackpotBalance = await adjustClubJackpotBalance(club.id, amount);
    await broadcastClub(club.code);
    ctx.reply({ ok: true, treasuryChips, jackpotBalance });
    return;
  }
  ...
  if (type === "spectate_club_table") {
    ...
    const rt = ensureRuntime(code, club.id);
    if (!rt.table) rt.table = new PokerTable({ smallBlind: t.small_blind, bigBlind: t.big_blind, rakePercent: Number(t.rake_percent), variant: t.variant, maxPlayers: t.max_players || 9 });
    rt.sockets.add(ws);
    rt.socketToPlayer.set(ws, ws.username);
    ctx.setJoinedCode(code);
    ctx.reply({ ok: true, code });
  ...
  if (type === "sit_club_table") {
    ...
    if (!rt.table) rt.table = new PokerTable({ smallBlind: t.small_blind, bigBlind: t.big_blind, rakePercent: Number(t.rake_percent), variant: t.variant, maxPlayers: t.max_players || 9 });
    // Assento escolhido na tela de espectador (botão "+" numa vaga
    // específica) — opcional, cai no primeiro livre se não vier.
    const seatChoice = Number.isInteger(msg.seat) ? msg.seat : null;
    rt.table.addPlayer(ws.username, ws.username, buyIn, false, seatChoice);
