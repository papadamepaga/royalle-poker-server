...
import {
  ...
  listClubTables, createClubTable, getClubTableById, recordPayLedger, getPayHistory,
} from "./store.js";
...
function publicClub(club, viewerIsOwner, extra = {}) {
  ...
  if (viewerIsOwner) {
    return { ...base, treasuryChips: Number(club.treasury_chips), totalRake: extra.totalRake ?? 0 };
  }
  return base;
}
...
async function handleMessage(ws, msg, ctx) {
  ...
  if (type === "join_club") {
    ...
    if (!existing) await addMember(club.id, ws.userId, 0, "member");
  ...
  if (type === "sit_club_table") {
    ...
    if (!member || Number(member.chips) < t.buy_in) return ctx.reply({ ok: false, error: "Royalle Pay insuficiente." });
    await adjustMemberChips(club.id, ws.userId, -t.buy_in);
    ...
    rt.table.addPlayer(ws.username, ws.username, t.buy_in, false);
    rt.sockets.add(ws);
  ...
  if (type === "find_table") {
    ...
    const wallet = await getOrCreateQuickWallet(ws.userId);
    if (wallet.chips < tier.buyIn) return ctx.reply({ ok: false, error: "Royalle Coins insuficientes pra esse nível." });
    ...
    await adjustQuickWalletChips(ws.userId, -tier.buyIn);
    rt.table.addPlayer(ws.username, ws.username, tier.buyIn, false);
