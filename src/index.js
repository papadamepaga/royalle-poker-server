...
function publicClub(club, viewerIsOwner, extra = {}) {
  ...
  const base = {
    ...
    rkBalance: Number(club.rk_balance || 0),
...
async function handleMessage(ws, msg, ctx) {
  ...
  if (type === "adjust_chips") {
    ...
    // na retirada. A tesouraria perde o valor cheio, o jogador recebe só
    // 95%, e os 5% viram taxa do app (mesmo mecanismo já usado pro rake
    // da mesa, só que aqui é sobre transferência de fichas mesmo).
    const sendFee = delta > 0 ? Math.floor(delta * 0.05) : 0;
    const netToPlayer = delta > 0 ? delta - sendFee : delta;
    const chips = await adjustMemberChips(club.id, target.id, netToPlayer);
    ...
    await recordPayLedger({
      ...
      amount: Math.abs(netToPlayer), balanceBefore, balanceAfter: Number(chips),
    });
