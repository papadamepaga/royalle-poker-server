...
import {
  ...
  createClub, getClubByCode, addMember, getMember, listMembers, listClubsForUser, setClubCover,
  adjustMemberChips, adjustClubTreasury, recordRake, getWeeklyRake, getTotalRake,
  createNotification, listNotifications, deleteNotification,
  getOrCreateQuickWallet, adjustQuickWalletChips, claimDailyBonus, buyDiamonds,
...
// Loja de diamantes — moeda premium fictícia, comprada com Royalle Coin
// (nunca dinheiro real). Preços em Royalle Coin, não em $.
const DIAMOND_SHOP = [
  { id: "d60", diamonds: 60, coinCost: 1000 },
  { id: "d300", diamonds: 300, coinCost: 4500 },
  { id: "d780", diamonds: 780, coinCost: 11000 },
  { id: "d1218", diamonds: 1218, coinCost: 16000 },
  { id: "d2988", diamonds: 2988, coinCost: 38000 },
  { id: "d6468", diamonds: 6468, coinCost: 80000 },
];
const STAKES_TIERS = [
  { smallBlind: 100, bigBlind: 200, buyIn: 20000 },
  { smallBlind: 500, bigBlind: 1000, buyIn: 100000 },
  { smallBlind: 1000, bigBlind: 2000, buyIn: 200000 },
  { smallBlind: 5000, bigBlind: 10000, buyIn: 1000000 },
];
const QUICK_VARIANTS = {
  holdem: { label: "Texas Hold'em", enabled: true },
  plo4: { label: "4-Card PLO", enabled: false },
  plo5: { label: "5-Card PLO", enabled: false },
  plo6: { label: "6-Card PLO", enabled: false },
};
...
async function broadcastClub(code) {
  ...
  for (const ws of rt.sockets) {
    ...
    send(ws, {
      type: "club_state",
      club: publicClub(club, viewerIsOwner, { totalRake }),
...
async function maybeRecordRake(rt) {
  if (rt.table?.pendingRake > 0) {
    ...
    // O dono precisa ver o rake acumulado atualizar na hora, sem precisar
    // sair e entrar de novo no clube — igual já acontece com o rake do clube.
...
function publicClub(club, viewerIsOwner, extra = {}) {
  ...
  if (viewerIsOwner) {
    return { ...base, treasuryChips: Number(club.treasury_chips), totalRake: extra.totalRake ?? 0, platformRake: Math.floor((extra.totalRake ?? 0) * 0.01) };
  }
  return base;
}
...
async function handleMessage(ws, msg, ctx) {
  ...
  if (type === "signup") {
    ...
    ws.userId = user.id; ws.username = user.username; ws.nickname = user.nickname || user.username;
    const token = signToken(user.id, user.username);
    ctx.reply({ ok: true, token, user: { id: user.id, username: user.username, avatar: user.avatar, nickname: user.nickname || null } });
    return;
  }
  ...
  if (type === "set_nickname") {
    ...
    if (nickname.length < 2 || nickname.length > 20) return ctx.reply({ ok: false, error: "Apelido precisa ter entre 2 e 20 caracteres." });
    const user = await setNickname(ws.userId, nickname);
    if (!user) return ctx.reply({ ok: false, error: "Não deu pra salvar o apelido." });
    ws.nickname = user.nickname;
    ctx.reply({ ok: true, user: { id: user.id, username: user.username, avatar: user.avatar, nickname: user.nickname } });
    return;
  }
  ...
  if (type === "authenticate") {
    ...
    ws.userId = user.id; ws.username = user.username; ws.nickname = user.nickname || user.username;
    ctx.reply({ ok: true, user: { id: user.id, username: user.username, avatar: user.avatar, nickname: user.nickname || null } });
    return;
  }
  ...
  if (type === "create_club") {
    ...
    const club = await createClub({
      ...
      coverId: msg.coverId || "rio",
    });
    await addMember(club.id, ws.userId, 0, "owner");
    await createNotification({
      userId: ws.userId, kind: "notice", title: `Bem-vindo ao ${club.name}!`,
      body: `Caro jogador: você criou um clube. Tenha uma ótima experiência no Royalle Poker!`,
    });
  ...
  if (type === "set_club_cover") {
    if (!requireAuth(ws, ctx)) return;
    const club = await getClubByCode((msg.code || "").toUpperCase());
    if (!club) return ctx.reply({ ok: false, error: "Clube não encontrado." });
    if (club.owner_id !== ws.userId) return ctx.reply({ ok: false, error: "Só o dono pode trocar a capa do clube." });
    await setClubCover(club.id, msg.coverId);
    ctx.reply({ ok: true });
    return;
  }
  ...
  if (type === "adjust_chips") {
    ...
    if (delta > 0) {
      const senderMe = await getMember(club.id, ws.userId);
      await createNotification({
        userId: target.id, kind: "club", title: "Fichas Recebidas!",
        body: `"${senderMe?.username || ws.username}" (ID: ${ws.userId}) enviou ${delta} fichas para você.`,
        fromLabel: club.name,
      });
    }
  ...
  if (type === "list_notifications") {
    if (!requireAuth(ws, ctx)) return;
    const rows = await listNotifications(ws.userId);
    ctx.reply({ ok: true, notifications: rows.map((n) => ({ id: n.id, kind: n.kind, title: n.title, body: n.body, fromLabel: n.from_label, createdAt: n.created_at })) });
    return;
  }

  if (type === "delete_notification") {
    if (!requireAuth(ws, ctx)) return;
    await deleteNotification(ws.userId, msg.id);
    ctx.reply({ ok: true });
    return;
  }
  ...
  if (type === "create_club_table") {
    ...
    const maxPlayers = Math.min(9, Math.max(2, Number(msg.maxPlayers) || 9));
  ...
  if (type === "sit_club_table") {
    ...
    rt.table.addPlayer(ws.username, ws.nickname || ws.username, buyIn, false);
    rt.sockets.add(ws);
  ...
  if (type === "sit_table") {
    ...
    rt.table.addPlayer(ws.username, ws.nickname || ws.username, msg.buyIn);
    rt.socketToPlayer.set(ws, ws.username);
    ctx.reply({ ok: true });
    await broadcastClub(club.code);
  ...
  if (type === "buy_diamonds") {
    if (!requireAuth(ws, ctx)) return;
    const item = DIAMOND_SHOP.find((i) => i.id === msg.itemId);
    if (!item) return ctx.reply({ ok: false, error: "Item não encontrado." });
    const result = await buyDiamonds(ws.userId, item.coinCost, item.diamonds);
    if (!result) return ctx.reply({ ok: false, error: "Royalle Coin insuficiente." });
    ctx.reply({ ok: true, chips: result.chips, diamonds: result.diamonds });
    return;
  }
  ...
  if (type === "add_chips_at_table") {
    ...
    // Só dá pra recomprar fichas fora de uma mão em andamento — no meio
    // de uma mão bagunçaria o pote.
    const midHand = rt.table.stage !== "idle" && rt.table.stage !== "showdown";
    if (midHand) return ctx.reply({ ok: false, error: "Espera a mão atual terminar." });
    const amount = Math.floor(Number(msg.amount) || 0);
    if (amount <= 0) return ctx.reply({ ok: false, error: "Valor inválido." });
    if (rt.isQuick) {
      const chips = await adjustQuickWalletChips(ws.userId, -amount);
      if (chips < 0) { await adjustQuickWalletChips(ws.userId, amount); return ctx.reply({ ok: false, error: "Royalle Coin insuficiente." }); }
      player.chips += amount;
      ctx.reply({ ok: true, tableChips: player.chips, walletChips: chips });
      broadcastTable(code);
      return;
    }
    const clubCode = code.includes("#") ? code.split("#")[0] : code;
    ...
    player.chips += amount;
    ctx.reply({ ok: true, tableChips: player.chips });
    broadcastTable(code);
    return;
  }
